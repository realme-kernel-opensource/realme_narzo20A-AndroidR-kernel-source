...
void batt_temp_zone_arr_init(struct oplus_chg_chip *chip)
{
...
#ifdef CONFIG_OPLUS_SPECIAL__DEL
...
#endif
	if(chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC){
		chg_debug(" QC write vbus = %d", vbus_limit);
		chip->chg_ops->vbus_voltage_write(vbus_limit);
	} else {
		chg_debug(" Others AC write 5V\n");
...
static OPLUS_CHG_9V2A_TBATT_STATUS batt_temp_9v2a_stat_check(int temp, struct oplus_chg_chip *chip)
{
	OPLUS_CHG_9V2A_TBATT_STATUS i;
...
static void battery_temp_9v2a_anti_shake_handle(struct oplus_chg_chip *chip)
{
	int i;
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	OPLUS_CHG_9V2A_TBATT_STATUS tbatt_status_9v2a;
...
#ifdef CONFIG_OPLUS_SPECIAL_DEL
	chip->chging_over_time = false;
	printk("oplus_chg_check_time_is_good_aging_mode\n");
	return true;
#endif //CONFIG_OPLUS_SPECIAL_DEL
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ops->get_ibus) && (chip->chg_ops->get_charging_enable && (chip->chg_ops->get_charging_enable() == true))) {
		chip->ibus = chip->chg_ops->get_ibus();
	} else {
		chip->ibus = -1;
	}
	if((chip->chg_ops->get_rtc_soc() < 0) && retry_counts < RETRY_COUNTS) {
	...
	if (ui_soc_cp_flag == 0) {
		...
		charger_xlog_printk(CHG_LOG_CRTI,
			"[Battery]oplus_chg_get_battery_data,\
			ui_soc = %d,soc_load = %d,soc = %d\n",
			chip->ui_soc,soc_load, chip->soc);
	}
	next:
	return;
}
...
static void battery_update(struct oplus_chg_chip *chip)
{
	...
	{
		iWakeUpDelay++;
		if(iWakeUpDelay >= OPLUS_CHGON_WAKEUP_DELAY_COUNT)
		{
			iWakeUpDelay = 0;
		}
		else
...
#ifndef OPLUS_FEATURE_CHG_BASIC
static bool battery_qc_temp_anti_shake_handle(struct oplus_chg_chip *chip)
{
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (val > 6) {
		...
		chip->limits.input_current_vooc_ma_high = cool_down_current_limit_normal[6 - 1];
		chip->limits.input_current_vooc_ma_warm = cool_down_current_limit_normal[6 - 1];
		chip->limits.input_current_vooc_ma_normal = cool_down_current_limit_normal[6 - 1];
		chip->cool_down_done = true;
	} else {
		...
		if (val < 6) {
			...
			chip->limits.qc_input_volt_charger_v = VBUS_LIMIT_9V;
			/* Brandon is 5V2A not support QC ,use default current when val=6 */
			if (!(chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC)) {
				chip->limits.input_current_charger_ma = chip->limits.default_input_current_charger_ma;
			}
		}
	...
	charger_xlog_printk(CHG_LOG_CRTI, "val->intval = [%04x], set cool_down = [%d], set vbus_limit = [%d].\n",
						val, chip->cool_down, chip->limits.qc_input_volt_charger_v);
}

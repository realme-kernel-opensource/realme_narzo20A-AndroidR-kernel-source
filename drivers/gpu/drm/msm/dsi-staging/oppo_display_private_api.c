...
int dsi_panel_cabc_mode(struct dsi_panel *panel, int mode) {
	...
	if(panel->novatek_flag || panel->ilitek_innolux_gg3_flag) {
		switch (mode) {
		case 0:
			...
			break;
		case 1:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_CABC_UI_MODE);
			if (rc) {
				pr_err("[%s] failed to send DSI_CMD_CABC_UI_MODE cmds, rc=%d\n",
						panel->name, rc);
			}
			break;
		case 2:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_CABC_STILL_MODE);
			if (rc) {
				pr_err("[%s] failed to send DSI_CMD_CABC_STILL_MODE cmds, rc=%d\n",
						panel->name, rc);
			}
			break;
		case 3:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_CABC_MOVING_MODE);
			if (rc) {
				pr_err("[%s] failed to send DSI_CMD_CABC_MOVING_MODE cmds, rc=%d\n",
						panel->name, rc);
			}
			break;
		default:
